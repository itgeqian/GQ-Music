...
import NewBatchImportDialog from "./form/newBatchImport.vue";
...
import Upload2 from "@iconify-icons/ri/upload-2-line";
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              ...
              <el-popconfirm :title="`是否确认删除歌曲：${row.songName}？此操作不可撤销。`" @confirm="handleDelete(row)">
                <template #reference>
          ...
          <el-dialog
            ...
            destroy-on-close
          >
            <NewBatchImportDialog
              :formInline="{
                title: '批量导入',
                artistId: form.artistId,
                artistName: treeData.find(item => item.value === form.artistId)?.label || ''
              }"
