...
defineOptions({ name: "SongComments" });
...
async function fetchData() {
  loading.value = true;
  try {
    const res: any = await getSongComments(songId.value);
    if (res?.code === 0) {
...
<template>
  <div class="w-full">
    ...
    <PureTableBar title="歌曲评论管理" :columns="columns" @refresh="fetchData">
      <template #buttons>
