...
defineOptions({ name: "PlaylistComments" });
...
function buildTree(list: any[]) {
  ...
  list.forEach(it => {
    ...
    byId[it.commentId] = { ...it, children: Array.isArray(it.children) ? it.children : [] };
  });
  Object.values(byId).forEach((it: any) => {
    const pid = Number(it.pCommentId || 0);
    if (pid > 0 && byId[pid]) byId[pid].children.push(it); else if (pid === 0) roots.push(it);
  });
  return roots;
}
...
function flatten(list: any[], depth = 0, out: any[] = []) {
  list.forEach(it => {
    out.push({ ...it, _depth: depth, _displayContent: (it.replyNickName ? `回复 @${it.replyNickName} ` : "") + (it.content || "") });
    if (Array.isArray(it.children) && it.children.length) flatten(it.children, depth + 1, out);
  });
  return out;
}
...
async function fetchData() {
  loading.value = true;
  try {
    const res: any = await getPlaylistComments(playlistId.value);
    if (res?.code === 0) {
...
onMounted(() => {
  if (!Number.isFinite(playlistId.value)) {
    message("歌单ID无效", { type: "warning" });
    router.back();
    return;
  }
  fetchData();
...
<template>
  <div class="w-full">
    ...
    <PureTableBar title="歌单评论管理" :columns="columns" @refresh="fetchData">
      <template #buttons>
      ...
      <template #default="{ size, dynamicColumns }">
        <pure-table :data="dataList" :columns="dynamicColumns" :size="size" :loading="loading" :pagination="pagination">
          <template #content="{ row }">
            <span :style="{ marginLeft: (row._depth||0)*16 + 'px' }">
              {{ row._displayContent || row.content }}
            </span>
