...
export function usePlaylist(tableRef: Ref) {
  ...
  const formRef = ref();
  const dataList = ref([]);
  const pinnedSet = ref<Set<number>>(new Set());
  ...
  async function onSearch() {
    try {
      // 拉取固定推荐列表
      const pinnedRes = await fetchPinned();
      if (pinnedRes?.code === 0 && Array.isArray(pinnedRes.data)) {
        pinnedSet.value = new Set(pinnedRes.data.map((it: any) => it.playlistId));
      } else {
      ...
      if (result.code === 0 && result.data && result.data.items) {
        ...
        dataList.value = result.data.items.map(item => ({
          ...
          isPinned: pinnedSet.value.has(item.playlistId)
        }));
      } else {
  ...
  async function fetchPinned() {
    ...
    return fetch("/admin/pinnedPlaylists", {
      method: "GET",
  ...
  async function doRecommend(row) {
    const res: any = await recommendPlaylist(row.playlistId, 100);
    if (res.code === 0) {
      message("已设为推荐", { type: "success" });
      row.isPinned = true;
    } else {
  ...
  async function doCancelRecommend(row) {
    const res: any = await cancelRecommendPlaylist(row.playlistId);
    if (res.code === 0) {
      message("已取消推荐", { type: "success" });
      row.isPinned = false;
    } else {
      message(res.message || "操作失败", { type: "error" });
    }
