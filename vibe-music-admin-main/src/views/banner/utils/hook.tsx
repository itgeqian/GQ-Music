...
import { ElMessageBox, ElSwitch, ElImage, ElSelect, ElOption } from "element-plus";
...
export function useBanner(formData: any, tableRef: Ref) {
  ...
  // 远程搜索专辑
  const remoteSearchAlbum = async (query: string) => {
    albumLoading.value = true;
    try {
      // 放大 pageSize，避免仅回显前 10 条
      const res: any = await getAlbumList({ pageNum: 1, pageSize: 200, title: query || "" });
      if (res?.code === 0 && res?.data?.items) {
        albumOptions.value = res.data.items.map((it: any) => ({
          label: `${it.albumId} - ${it.title}`,
          value: it.albumId
        }));
      } else {
        albumOptions.value = [];
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      prop: "albumId",
      minWidth: 160,
      cellRenderer: ({ row }) => (<span>{row.albumId ? `专辑 #${row.albumId}` : '未设置'}</span>)
    },
    {
      label: "操作",
      fixed: "right",
            ...
            h(
              ...
              [
                ...
                latestInfo.value
                  ...
                  : null,
                // 关联设置：仅关联专辑ID（后续可换选择器）
                h(
                  'div',
                  { class: 'w-full mt-4 space-y-2' },
                  [
                    h('label', { class: 'text-sm text-gray-600' }, '关联专辑'),
                    h(ElSelect as any, {
                      filterable: true,
                      remote: true,
                      remoteMethod: remoteSearchAlbum,
                      loading: albumLoading.value,
                      placeholder: '输入专辑标题搜索并选择',
                      clearable: true,
                      modelValue: latestInfo.value?.albumId ?? (row?.albumId ?? undefined),
                      'onUpdate:modelValue': (v: any) => {
                        latestInfo.value = { ...(latestInfo.value || {}), albumId: v || undefined };
                      },
                      style: 'width:100%'
                    }, () =>
                      albumOptions.value.map(opt =>
                        h(ElOption as any, { key: opt.value, label: opt.label, value: opt.value })
                      )
                    )
                  ]
                )
