...
public interface UserFavoriteMapper extends BaseMapper<UserFavorite> {
    ...
    // 查询用户收藏的所有专辑ID
    @Select("SELECT album_id FROM tb_user_favorite WHERE user_id = #{userId} AND type = 3 ORDER BY create_time DESC")
    List<Long> getUserFavoriteAlbumIds(@Param("userId") Long userId);
