...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result<UserVO> userInfo() {
        ...
        // 避免缓存/CDN错误持久化：该接口不缓存
        return Result.success(userVO);
    }

    /** 公开：根据用户ID获取用户资料（含粉丝/关注数） */
    @Override
    public Result<UserProfileVO> getUserProfile(Long profileUserId) {
        User user = userMapper.selectById(profileUserId);
        if (user == null) {
            return Result.error(MessageConstant.USER + MessageConstant.NOT_EXIST);
        }

        long fans = userFollowMapper.countFans(profileUserId);
        long followings = userFollowMapper.countFollowings(profileUserId);

        Boolean followedByMe = null;
        try {
            // 优先从 ThreadLocal（拦截器）读取；若未拦截（公开接口），则从请求头解析 token
            Long meId = null;
            try {
                Map<String, Object> local = ThreadLocalUtil.get();
                meId = TypeConversionUtil.toLong(local.get(JwtClaimsConstant.USER_ID));
            } catch (Exception ignored) {}

            if (meId == null || meId == 0L) {
                ...
                if (attrs != null) {
                    ...
                    if (token != null && !token.isEmpty()) {
                        ...
                        if (claims != null) {
                            meId = TypeConversionUtil.toLong(claims.get(JwtClaimsConstant.USER_ID));
                        }
                    }
                }
            }

            if (meId != null && meId > 0 && !meId.equals(profileUserId)) {
                followedByMe = userFollowMapper.exists(meId, profileUserId) > 0;
            }
        } catch (Exception ignored) {}
        ...
        vo.setFans(fans);
        vo.setFollowings(followings);
        vo.setPrivateUser(Boolean.FALSE.equals(user.getPublicProfile()) ? Boolean.TRUE : Boolean.FALSE);
    ...
    public Result<String> followUser(Long targetUserId) {
        Map<String, Object> map = ThreadLocalUtil.get();
        Long meId = TypeConversionUtil.toLong(map.get(JwtClaimsConstant.USER_ID));
        if (meId.equals(targetUserId)) return Result.error(MessageConstant.OPERATION + MessageConstant.FAILED);
        if (userFollowMapper.exists(meId, targetUserId) > 0) {
            return Result.error(MessageConstant.ADD + MessageConstant.FAILED);
        }
        UserFollow uf = new UserFollow().setFollowerId(meId).setFolloweeId(targetUserId).setCreateTime(java.time.LocalDateTime.now());
    ...
    /** 取消关注用户 */
    @Override
    public Result<String> cancelFollowUser(Long targetUserId) {
        Map<String, Object> map = ThreadLocalUtil.get();
        Long meId = TypeConversionUtil.toLong(map.get(JwtClaimsConstant.USER_ID));
        com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<UserFollow> qw = new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<>();
        qw.eq("follower_id", meId).eq("followee_id", targetUserId);
        int rows = userFollowMapper.delete(qw);
        if (rows == 0) return Result.error(MessageConstant.DELETE + MessageConstant.FAILED);
        return Result.success(MessageConstant.DELETE + MessageConstant.SUCCESS);
    }

    /** 是否已关注用户 */
    @Override
    public Result<Boolean> isUserFollowed(Long targetUserId) {
        Map<String, Object> map = ThreadLocalUtil.get();
        Long meId = TypeConversionUtil.toLong(map.get(JwtClaimsConstant.USER_ID));
        boolean exists = userFollowMapper.exists(meId, targetUserId) > 0;
        return Result.success(exists);
    }

    /** 我关注的用户列表 */
    @Override
    public Result<List<UserVO>> getFollowing(Long userId) {
        Long queryUserId = userId;
        if (queryUserId == null) {
            Map<String, Object> map = ThreadLocalUtil.get();
            queryUserId = TypeConversionUtil.toLong(map.get(JwtClaimsConstant.USER_ID));
        }
        List<Long> ids = userFollowMapper.findFolloweeIds(queryUserId);
        if (ids == null || ids.isEmpty()) return Result.success(java.util.Collections.emptyList());
        List<User> users = userMapper.selectBatchIds(ids);
        java.util.List<UserVO> list = new java.util.ArrayList<>();
        for (User u : users) {
            if (u == null) continue;
    ...
    /** 关注我的用户列表 */
    @Override
    public Result<List<UserVO>> getFollowers(Long userId) {
        Long queryUserId = userId;
        if (queryUserId == null) {
            Map<String, Object> map = ThreadLocalUtil.get();
            queryUserId = TypeConversionUtil.toLong(map.get(JwtClaimsConstant.USER_ID));
        }
        List<Long> ids = userFollowMapper.findFollowerIds(queryUserId);
        if (ids == null || ids.isEmpty()) return Result.success(java.util.Collections.emptyList());
        List<User> users = userMapper.selectBatchIds(ids);
        java.util.List<UserVO> list = new java.util.ArrayList<>();
        for (User u : users) {
