...
// 全局事件：凡是出现“请先登录”提示的场景，同时弹出登录框
onMounted(() => {
  const openAuth = () => { authVisible.value = true }
  window.addEventListener('auth:show', openAuth, { once: false })
  onBeforeUnmount(() => window.removeEventListener('auth:show', openAuth))
})
...
const handleProtectedRoute = (path: string) => {
  const needsAuth = ['/like', '/user', '/recent', '/my-playlists', '/profile']
  if (!user.isLoggedIn && needsAuth.some(p => path === p || path.startsWith(p + '/'))) {
    // 仅统一弹一次提示 + 登录弹窗
...
<template>
  <aside class="w-64 hidden h-full overflow-hidden md:block border-transparent bg-transparent">
    <nav class="flex flex-col p-4 space-y-4 flex-1 h-full box-border overflow-hidden">
      <div v-for="(item, index) in filteredMenu" :key="index" class="w-full flex flex-col gap-1">
      ...
      <!-- 收藏的歌单 -->
      <div class="w-full flex flex-col gap-1 flex-1 min-h-0" v-if="user.isLoggedIn">
        <h3 class="ml-4 text-sm font-semibold text-inactive">收藏的歌单（{{ favoriteStore.favoritePlaylists.length }}）</h3>
        <el-scrollbar class="flex-1" style="max-height: 300px;">
