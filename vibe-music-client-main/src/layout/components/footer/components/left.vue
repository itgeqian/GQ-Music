...
// 监听全局事件：需要在跳转前收起播放器抽屉
onMounted(() => {
  const close = () => { showDrawerMusic.value = false }
  window.addEventListener('drawer:close', close)
  onBeforeUnmount(() => window.removeEventListener('drawer:close', close))
})
</script>
