  ...
  try {
    ...
    if (res.code === 0) {
      playlists.value = res.data.items.map(item => ({
        ...
        creator: {
          nickname: selected.value === 'favorite' ? 'GQ Music' : 'GQ Music',
          avatarUrl: coverImg
        },
        playCount: 0,
        subscribedCount: 0
...
<template>
  <div class="flex flex-col h-full flex-1 overflow-hidden px-4 py-2">
    ...
    <div class="flex-grow flex flex-col overflow-x-hidden cursor-pointer">
      <div class="border-0 p-0 m-0 bg-transparent">
        <div
          class="inline-flex h-10 items-center rounded-lg p-1 text-muted-foreground w-full justify-start mb-2 overflow-x-auto"
